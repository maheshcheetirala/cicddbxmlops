...
```
dbx execute --cluster-name=<name of interactive cluster> --job=mlopsdbx-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=mlopsdbx-sample-integration-test --files-only
dbx launch --job=mlopsdbx-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `mlopsdbx/__init__.py` file and tag the current code version:
```
